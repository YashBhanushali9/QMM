...
```{r}
heartstarttable <- matrix(c(22,14,30,600,100 ,
16,20,24,625,120,
80,60,70,"-","-" ), ncol=5, byrow=TRUE)
colnames(heartstarttable) <- c("Warehouse 1", "Warehouse 2", "Warehouse 3","Production cost","Production Capacity")
rownames(heartstarttable) <- c("Plant A", "Plant B","Monthly Demand")
heartstarttable <- as.table(heartstarttable)
heartstarttable
```
...
The production of 80 AEDs in Plant 2 - Warehouse1, 60 AEDs in Plant 1 - Warehouse2, 40 AEDs in Plant 1 - Warehouse3, and 30 AEDs in Plant 2 - Warehouse3 should be distributed to each of the three wholesaler warehouses in order to reduce the overall cost of manufacturing and shipping.
